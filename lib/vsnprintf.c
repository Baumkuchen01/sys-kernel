/**
 * The contents of this file are derived from musl libc, tailored for Sys2.
 * Handling of floating point numbers and wide characters have been removed.
 *
...
// the original vfprintf from musl libc
...
int str_write(FILE *f, const void *buf, size_t len) {
  if (!f->str || !f->size)
    return 0;
  if (f->offset + len >= f->size)
    len = f->size - f->offset - 1;
  if (len > 0) {
    for (char *s = (char *)buf; len--; s++) {
      f->str[f->offset] = *s;
      f->offset++;
    }
  }
...
int vsnprintf(char *str, size_t size, const char *restrict fmt, va_list ap) {
  FILE snprintf_out = {
    .str = str,
    .offset = 0,
    .size = size,
  ...
  ret = printf_core(&snprintf_out, fmt, &ap, nl_arg, nl_type);
  if (snprintf_out.offset < size)
    str[snprintf_out.offset] = '\0'; 
  else if (size > 0)
    str[size - 1] = '\0';
  return ret;
}
...
int sprintf(char *str, const char *fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
  int ret = vsnprintf(str, 0x3f3f3f3f, fmt, ap);
  va_end(ap);
  return ret;
}
